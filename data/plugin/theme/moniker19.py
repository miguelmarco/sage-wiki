...
# ## Rick This is a basic, no-frills (no hiding) panel
    def navibarpanel(self, d):
        ...
        html = [
            u'<div class="sidepanel"><h1>%s</h1>' % ( _("Marcadores")),
            self.navibar(d),
            u'</div>',
            ]
        return u''.join(html) 
...
# Rick: Borrowing code from Roger Haase's FixedLeft theme so that the searchbox buttons will wrap: 
    def searchpanel(self,d):
        ...
        _ = self.request.getText
        html = [
            u'<div class="sidepanel"><h1>%s</h1>' % (_("Buscar en la wiki")),
            self.searchform(d),
            u'</div>',
            ]
        return u''.join(html)
    ...
    def trailpanel(self, d):
        ...
        if trail:
            html = [
                u'<div class="sidepanel"><h1>%s</h1>' % ( _("Recientes")),
                trail,
                u'</div>',
                ]
        else:
    ...
    def editbarpanel(self, d):
        ...
        if self.shouldShowEditbar(d['page']):
            html = [
                u'<div class="sidepanel"><h1>%s</h1>' % ( _("Herramientas")),
                self.editbar(d),
                u'</div>',
                ]
            return u''.join(html)
...
# Rick:  Use the WhatLinksToThis.py macro available on http://moinmo.in/MacroMarket
    def title_with_separators(self, d):
        ...
        return u'<span id="pagelocation">ubicaci&oacute;n: %s</span>' % html
    ...
    def username(self, d):
        ...
        if request.user.valid and request.user.name:
            ...
            if 'userprefs' not in self.request.cfg.actions_excluded:
                userlinks.append(d['page'].link_to(request, text=_('preferencias'),
                                               querystr={'action': 'userprefs'}, id='userprefs', rel='nofollow'))

        if request.user.valid:
            if request.user.auth_method in request.cfg.auth_can_logout:
                userlinks.append(d['page'].link_to(request, text=_('salir'),
...
#
    ...
    def attachmentsLink(self, page):
        ...
        _ = self.request.getText
        return page.link_to(self.request,
                            text=_('cargar y gestionar archivos'),
    ...
    def quicklinkLink(self, page):
        ...
        if self.request.user.isQuickLinkedTo([page.page_name]):
            ...
            action, text = 'quicklink', _("agregar a marcadores")
        if action in self.request.cfg.actions_excluded:
            return ""
